...
public class TrainerRegistrationSteps {
    ...
    public void setUp() {
        ...
        mockMvc = standaloneSetup(trainerController).setValidator(validator).setControllerAdvice(new BaseExceptionHandler(), new MethodArgumentNotValidExceptionHandler()).build();
    }
    ...
    public void validTrainerRegistrationData() {
        ...
        when(trainerService.createTrainer(any(NewTrainerRequestDTO.class))).thenReturn(response);
    }
    ...
    public void theTrainerRegistrationRequestIsSent() throws Exception {
        result = mockMvc.perform(post("/api/trainers").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andReturn();
    }

    @Then("the trainer registration response status should be {int}")
    public void theTrainerRegistrationResponseStatusShouldBe(int expectedStatus) {
        assertThat(result.getResponse().getStatus()).isEqualTo(expectedStatus);
    }

    @Then("generated trainer credentials should be returned")
    public void generatedTrainerCredentialsShouldBeReturned() throws Exception {
        JsonNode response = objectMapper.readTree(result.getResponse().getContentAsString());
        assertThat(response.get("username").asText()).isEqualTo("Daniel.Anderson");
        assertThat(response.get("password").asText()).isEqualTo("password123");
        verify(trainerService).createTrainer(any(NewTrainerRequestDTO.class));
    }

    @Then("trainer registration service should not be called")
    public void trainerRegistrationServiceShouldNotBeCalled() {
        verify(trainerService, never()).createTrainer(any(NewTrainerRequestDTO.class));
    }
}